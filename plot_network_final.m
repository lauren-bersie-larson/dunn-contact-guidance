function plot_network_final(nodes, fibers, fibseg_nodes, direction, h, k, l, R, L)
...
for n = 1 : num_fibers
    ...
    if ismember(fibers(n,1), fibseg_nodes) && ismember(fibers(n,2), fibseg_nodes)
        plot3( x, y, z, 'r' );
    else
        plot3( x, y, z, 'b' );
    end
...
if 1==1
    ...
    switch direction 
        ...
        case 3
            X = R.* cos(A)+h;
            Y = R.* sin(A)+k;
            Z = meshgrid(linspace(-0.5, -0.25+L, 20), linspace(-0.5, -0.25+L, 20))';
    end
